...
\label{tab:robots}
\centering
% TODO robot parameter table, to be added manually as stated
\begin{tabular}{lccc}
\toprule
...
Across the nine settings we compared the heat map predicted for each robot against the observed behaviour of the physical platform. Every staged test case was attempted by the robot and recorded on video, the executed motion was annotated onto the map, and a prediction counts as correct when a region marked accessible was traversed and a region marked inaccessible could not be entered. Fig.~\ref{fig:heatmaps} shows representative settings with the predicted heat map beside the annotated behaviour of the robot.
% TODO per-setting outcomes, number of staged cases, agreement counts, and
% the index per robot per setting once the annotation comparison is final
\textit{[Quantitative outcomes to be added, agreement per test case across the nine settings and the index per robot per setting.]}
 
The comparisons follow a consistent pattern. \textit{[Overall agreement sentence to be added, prediction and behaviour agreed in X of Y staged cases.]} Obstacles placed at the edge of a route were correctly left accessible, passages narrower than the inflated footprint and blocking partitions were correctly marked unreachable, and the indices separated the three platforms in the order their capabilities predict. % TODO confirm these qualitative observations against the final annotation set
 
\begin{figure*}[t]
\centering
% TODO replace placeholder with panels, predicted heat map beside annotated
% robot behaviour, representative settings, one row per robot
\framebox[\textwidth]{\rule{0pt}{4cm}\parbox{0.9\textwidth}{\centering Placeholder. Predicted heat maps beside annotated robot behaviour for representative settings and all three target robots.}}
\caption{Predicted accessibility heat maps compared with observed robot behaviour in representative environment settings. Regions predicted accessible and unreachable are distinguished by colour, and the annotated trajectory shows the motion the physical robot achieved.}
\label{fig:heatmaps}
\end{figure*}
