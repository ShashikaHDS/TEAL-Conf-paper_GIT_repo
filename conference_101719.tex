...
We assessed two target robots chosen for their deliberately different forms, an omnidirectional transport robot (Movilo) and a quadruped (Unitree GO2).
% NOTE the BES FT-P320 painting robot was scanned for but its assessment is
% not yet complete; reinstate it here and in Table II once its data is final
...
\label{tab:robots}
\centering
% TODO fill the GO2 height, step and slope tolerances, and the Movilo slope
% tolerance; Movilo values from the project report (height loaded with goods)
\begin{tabular}{lcc}
\toprule
Parameter & Movilo & GO2 \\
\midrule
Length (L) & 0.98\,m & 0.70\,m \\
Width (W) & 0.95\,m & 0.40\,m \\
Height (H) & 0.50\,m & x \\
Maximum climbable step ($s_{\max}$) & 0.045\,m & x \\
Maximum climbable slope ($\theta_{\max}$) & x & x \\
...
% TODO reinstate this figure once the before and after relocation heat maps
% exist, and restore its reference in the paragraph above
% \begin{figure}[!t]
% \centering
% \includegraphics[width=\linewidth]{figs/beforeafter.png}
...
This paper introduced the Robot Inclusivity Index, a closed-loop framework that turns a single point-cloud walkthrough of a construction site into a deployment assessment for any mobile robot, producing a robot-specific traversable map, an accessibility map, a single-figure index, and a ranked list of site changes with predicted gains. In a field study at an active construction site, the accessibility maps predicted for an omnidirectional and a legged platform matched the behaviour annotated from deployment video on 92.6\% of the labelled area, rising to 97.9\% once annotation boundary effects are discounted, 58 of 60 staged test conditions were predicted correctly, and the guided-relocation loop was exercised end to end, from barrier diagnosis through operator-refined planning to physical changes verified by rescanning. % TODO add the measured relocation gains once Table~\ref{tab:relocation} is filled
Future work will extend validation to additional sites and larger intervention studies, and broaden the index beyond accessibility to the other dimensions of robot inclusivity.
